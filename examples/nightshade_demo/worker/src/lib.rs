...
fn apply_client_message(world: &mut World, scene: &mut Scene, message: ClientMessage) {
    match message {
        ...
        ClientMessage::SetSpinSpeed { speed } => {
            scene.spin_speed = speed;
        }
        ClientMessage::SpawnCube => {
            systems::example::spawn_cube_on_ring(scene, world);
        }
        ClientMessage::SpawnSphere => {
            systems::example::spawn_sphere_on_ring(scene, world);
        }
        ClientMessage::SetBackgroundPreset { preset } => {
            systems::example::set_scene_background(world, &preset);
        }
        ClientMessage::SetBackgroundColor { red, green, blue } => {
            systems::example::set_background_color(world, [red, green, blue]);
        }
        ClientMessage::SetSelectedColor { red, green, blue } => {
            systems::example::recolor_selected(scene, world, [red, green, blue]);
        }
        ClientMessage::SetSelectedScale { scale } => {
            systems::example::rescale_selected(scene, world, scale);
        }
